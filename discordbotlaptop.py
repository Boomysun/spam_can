...
def word_search(word_original):
    if word_original == 1:
        ...
        for word in word_original:
                if word != word_original[0]:
                    word_to_find = word_to_find + " " + word
        vgm_url = 'https://www.urbandictionary.com/define.php?term=' + word_to_find
...
async def on_message(message):
    ...
    if "The Fam" == channelname:
        ...
        if message.content == "$Menu":
            ...
            urbanembeded = word_search(message.content)
            await message.channel.send(embed = urbanembeded)
...
client.run('ODc5ODM0OTU0NTEzNjU3ODY2.YSVgJw.6T7x41V7W0ETv-n-RYy9VhEsn8g')